"""Render provider-masked enhancement for the homepage and investigation maps, without smoothing or thresholding."""
...
scales={o['id']:o['asset']['scale'][1] for o in snapshot['observations']}
for path in sorted((root/'research').glob('*/*_con_tif.tif')):
 ...
 with rasterio.open(path) as src:
  ...
  indices=np.minimum((np.nan_to_num(dest,nan=0).clip(0,maximum)/maximum*256).astype(int),255)
  ...
  catalog=json.loads((path.parent/f'{name}.json').read_text())
  asset={'href':catalog['items'][0]['con_tif']}
  manifest[name]={'url':f'/data/home-plumes/{name}.png','coordinates':[[west,north],[east,north],[east,south],[west,south]],'bounds':[west,south,east,north],'unit':'ppm·m','scale':[0,maximum],'sourceUrl':asset['href'],'sourceSha256':hashlib.sha256(path.read_bytes()).hexdigest(),'sha256':hashlib.sha256(png).hexdigest(),'nativeSamples':int(raw.count()),'resampling':'nearest','mask':'Provider nodata mask; transparent areas are not measured zeros'}

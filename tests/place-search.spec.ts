...
test('homepage ZIP selection opens its own map and survives reload',async({page})=>{
 ...
 await expect(page).toHaveURL(/\/places\/us-24058$/);await expect(page.getByRole('heading',{name:'Monroe Township 08831',exact:true})).toBeVisible();await expect(page.getByRole('region',{name:'Search map centered on Monroe Township'})).toBeVisible();await expect(page.getByRole('heading',{name:'No case or regional records nearby'})).toBeVisible();await expect(page.getByRole('dialog')).toHaveCount(0);
...
test('Santiago shows sourced regional detections and selected footprints',async({page})=>{
 ...
 await expect(page.locator('.place-results-title')).toContainText('30 case / regional records');
 ...
 await expect(page.locator('.place-results-title')).toContainText('101 case / regional records');

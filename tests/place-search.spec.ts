...
test('homepage ZIP selection opens its own map and survives reload',async({page})=>{
 ...
 const input=page.getByRole('textbox',{name:'City, region, or US ZIP code'});
 await input.fill('08831');await expect(page.getByRole('button',{name:/Monroe Township.*08831/})).toBeVisible();await input.press('Enter');
...
test('nearby evidence opens the real investigation and empty date filters can reset',async({page})=>{
 await page.goto('/');await page.locator('.home-search').click();await page.getByRole('textbox',{name:'City, region, or US ZIP code'}).fill('95035');await page.getByRole('button',{name:/Milpitas.*95035/}).click();

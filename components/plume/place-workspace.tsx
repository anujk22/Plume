...
export default function PlaceWorkspace({placeId,initialRadius}:{placeId:string;initialRadius:number}){
 ...
 return <div className="application investigation-application place-application"><Header place={label} onSearch={()=>setSearch(true)}/><div className="workspace-art" aria-hidden="true"/><WorkspaceForeground/><main className="workspace place-workspace">
  ...
  <aside className="findings-panel place-results" aria-label="Search results"><p className="eyebrow">PUBLISHED METHANE OBSERVATIONS</p><h2>{visible.length?'Methane comes into view.':'Beyond this snapshot.'}</h2>{catalogError&&<p role="alert">{catalogError}</p>}{visible.length?<><p className="place-result-summary">{visible.length} observations · {new Set(visible.map(o=>o.date.slice(0,10))).size} dates · within {radius} km</p>{cases.map(c=>{const records=snapshot.observations.filter(o=>o.caseId===c.id&&visible.some(v=>v.id===o.id));return <Link className="place-result-card" key={c.id} href={'/investigations/'+c.id+'?observation='+records[0].id}><img src={records[0].asset.url} alt=""/><div><h3>{c.place}</h3><p>{records.length} observations · {new Set(records.map(o=>o.date.slice(0,10))).size} dates</p><small>{dateLabel(records[0].date,true)} — {dateLabel(records.at(-1)!.date,true)}</small><p>Nearest included origin: {Math.min(...records.map(o=>distanceKm(place.location,o.location))).toFixed(1)} km</p></div><ArrowRight size={17}/></Link>})}</>:<div className="place-no-results"><Info size={27}/><h3>No case or regional records nearby</h3><p>{nearby.length?'The selected date range excludes the nearby observations.':'Our case and regional catalog contains no records within '+radius+' km of '+label+'. It covers selected investigations and the Santiago region. The separate NASA map layer may contain detections here.'}</p><p>This does not establish that methane emissions are absent.</p>{nearby.length>0&&<button className="text-link" onClick={()=>{setFrom('');setTo('');}}>Reset date range</button>}</div>}

...
export default function PlaceWorkspace({placeId,initialRadius}:{placeId:string;initialRadius:number}){
 ...
 return <div className="application investigation-application place-application"><Header place={label} onSearch={()=>setSearch(true)}/><div className="workspace-art" aria-hidden="true"/><main className="workspace place-workspace">
  <aside className="case-rail place-filters" aria-label="Search filters"><p className="eyebrow">METHANE EXPLORER</p><h1>{label}</h1><p className="place-region">{place.admin}, {place.country}</p><button className="place-action" onClick={()=>setSearch(true)}><Search size={16}/> Search another place <ArrowRight size={14}/></button>
   ...
   <div className="place-data-source"><Layers3 size={18}/><div><strong>Data source</strong><p>Source: Carbon Mapper</p><small>Satellite & airborne · historical records</small></div></div>
  ...
  <section className="place-map-column"><div className="place-results-title"><MapPin size={17}/><span>Showing results for <strong>{label}</strong></span><span>{visible.length} observations</span></div><div className="place-map-stage"><PlaceMap place={place} radius={radius} observations={visible} selected={selected} onObservation={openObservation}/></div><div className="place-results-footer"><Info size={16}/><p>{visible.length?`${visible.length} published observations on ${new Set(visible.map(o=>o.date.slice(0,10))).size} dates. Gold marks detections. Numbered circles group nearby records; select a record to see its footprint.`:'This snapshot does not cover every location. An empty search is not evidence of zero methane.'}</p></div></section>
  <aside className="findings-panel place-results" aria-label="Search results"><p className="eyebrow">PUBLISHED METHANE OBSERVATIONS</p><h2>{visible.length?'Follow the detections.':'Beyond this snapshot.'}</h2>{catalogError&&<p role="alert">{catalogError}</p>}{visible.length?<><p className="place-result-summary">{visible.length} observations · {new Set(visible.map(o=>o.date.slice(0,10))).size} dates · within {radius} km</p>{cases.map(c=>{const records=snapshot.observations.filter(o=>o.caseId===c.id&&visible.some(v=>v.id===o.id));return <Link className="place-result-card" key={c.id} href={'/investigations/'+c.id+'?observation='+records[0].id}><img src={records[0].asset.url} alt=""/><div><h3>{c.place}</h3><p>{records.length} observations · {new Set(records.map(o=>o.date.slice(0,10))).size} dates</p><small>{dateLabel(records[0].date,true)} — {dateLabel(records.at(-1)!.date,true)}</small><p>Nearest included origin: {Math.min(...records.map(o=>distanceKm(place.location,o.location))).toFixed(1)} km</p></div><ArrowRight size={17}/></Link>})}</>:<div className="place-no-results"><Info size={27}/><h3>No included observations nearby</h3><p>{nearby.length?'The selected date range excludes the nearby observations.':'Our bundled catalog contains no records within '+radius+' km of '+label+'. It covers selected investigations and the Santiago region, not the whole world.'}</p><p>This does not establish that methane emissions are absent.</p>{nearby.length>0&&<button className="text-link" onClick={()=>{setFrom('');setTo('');}}>Reset date range</button>}</div>}
   {selected&&regional.length>0&&<section className="catalog-selection" aria-label="Selected catalog observation">
    ...
    <h3>Selected observation</h3><p>The gold outline on the map is this record’s published plume footprint.</p>
    <dl><div><dt>Location</dt><dd>{Math.abs(selected.location[1]).toFixed(3)}° S · {Math.abs(selected.location[0]).toFixed(3)}° W</dd></div><div><dt>Source attribution</dt><dd>Not assessed</dd></div><div><dt>Emission rate</dt><dd>Not included in this excerpt</dd></div></dl>
